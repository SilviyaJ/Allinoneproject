...
export const onSubmitShortList=async(object)=>{
    try{
        if(object.model!=""&&object.size==0.0&&object.brand==""){
            const t = await axios.get(`/api/bygen/${object.model}`,{
                headers:{
                    "Authorization":`Basic ${sessionStorage.getItem('user')}`
                }
            })
        ...
        else if(object.model==""&&object.size!=0.0&&object.brand!=""){
